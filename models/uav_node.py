...
class UAVNode(BaseNode):
    ...
    def get_processing_capacity(self) -> float:
        ...
        bits_capacity = (self.state.cpu_frequency * delta_t * battery_factor) / compute_density
    ...
    def calculate_energy_consumption(self, processing_time: float) -> float:
        """
        计算UAV处理阶段能耗（仅包含任务执行功耗，悬停/静态能耗在step中统一结算）
        """
        compute_energy = self.kappa3 * (self.state.cpu_frequency ** 2) * processing_time
        
        # 鏇存柊鐢垫睜鐢甸噺 (鍙璁＄畻鑳借€)
        self._update_battery_level(compute_energy)
        
        # 鏇存柊鍔熻€楃姸鎬?
        if processing_time > 0:
            self.state.current_power = compute_energy / processing_time
        else:
            self.state.current_power = 0.0
    ...
    def step(self, time_step: float, vehicle_positions: Optional[Dict] = None) -> List[Task]:
        ...
        self._record_energy_usage(hover_energy)
