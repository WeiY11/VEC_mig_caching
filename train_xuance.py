...
使用方式：
    # 使用默认TD3配置训练
    python train_xuance.py
    
    # 指定算法和配置文件
    python train_xuance.py --method td3 --config xuance_configs/td3_vec.yaml
    
    # 命令行覆盖参数
    python train_xuance.py --method sac --episodes 500 --device cuda:0
    
    # 测试模式
    python train_xuance.py --method td3 --test --model-path ./models/td3_vec/
...
def main():
    ...
    if args.config:
        ...
        # 尝试加载默认配置文件
        default_config_path = f"xuance_configs/{args.method.lower()}_vec.yaml"
