...
图~\ref{fig:system_architecture} 所示区域为一个“井”字型（九宫格）城市微场景，两条主干道路正交并延伸形成规则格局。系统中共有\textbf{6 个固定 RSU} 与 \textbf{2 架固定悬停无人机}，部署与功能如下：
\begin{enumerate}
    \item \textbf{左上角 RSU}: 位于左上道路交叉口外侧路缘，覆盖该角落进入/转弯的车辆流量，提供基础卸载入口。
    \item \textbf{右上角 RSU}: 位于右上交汇处外侧，同时作为右半区域与空域链路锚点，支撑右上车辆及与附近无人机的地空通信。
    \item \textbf{左下角 RSU}: 部署在左下交叉口外侧，服务该路段的进出与等待车辆，减少其接入时延。
    \item \textbf{右下角 RSU}: 位于右下交叉口外侧，为右下区域车辆提供边缘接入与任务卸载处理。
    \item \textbf{中央 RSU}: 更靠近城市核心高楼集中区（任务/数据高产热点），具备较高计算与缓存能力，用于承载高并发卸载与结果集中缓存复用。
    \item \textbf{中下位置 RSU}: 位于中心偏下方主干道路节点，起到纵向接入链路“补丁/平衡”作用，缓解中央 RSU 的峰值排队压力。
    \item \textbf{左上无人机 (UAV-A)}: 固定悬停在左上交叉口上空，不进行水平巡航。其作用：为左上区域车辆补充覆盖、承接高优先级或突发任务卸载、在角落 RSU 拥塞时分流部分计算。
    \item \textbf{右下无人机 (UAV-B)}: 悬停于右下侧高楼上空（略偏下），在高楼遮挡条件下提供空中中继/“准移动基站”功能，增强链路视距条件，并为时延敏感迁移任务提供备用路径。
\end{enumerate}

	extbf{缓存角色划分}: 中央 RSU 负责高复用结果集中缓存；外围四角及中下 RSU 仅缓存命中概率高的轻量结果；无人机只保留少量极低时延推理所需的关键中间/最终结果，避免其有限能量与存储被占满。
...
我们的优化目标是最小化在时间跨度 $T$ 内（包含多个时隙）系统总成本的长期期望值。该成本函数是一个加权和，包含了任务时延、系统能耗以及因各种原因（如超时、队列溢出）导致的数据丢失。

设 $\mathcal{J}_{succ}^t$ 为在时隙 $t$ 成功完成的任务集合，$\mathcal{J}_{dropped}^t$ 为在时隙 $t$ 被丢弃的任务集合。优化问题 (P1) 可以表述为：

\begin{equation}
\label{eq:objective_function_data_loss}
\min_{\mathbf{X}, \mathbf{B}, \mathbf{Z}, \mathbf{D}} \lim_{T \to \infty} \frac{1}{T} \mathbb{E} \left[ \sum_{t=0}^{T-1} \left( \omega_T \sum_{j \in \mathcal{J}_{succ}^t} T_{total,j} + \omega_E E_{total}^t + \omega_D \sum_{j \in \mathcal{J}_{dropped}^t} D_j \right) \right]
\end{equation}
其中：
\begin{itemize}
    \item $\mathbf{X} = \{x_{j,n}^t\}$, $\mathbf{B} = \{b_{link}^t\}$, $\mathbf{Z} = \{z_{j,r}^t\}$, $\mathbf{D} = \{D_{\cdot}^t\}$ 分别代表所有时隙内的任务分配、带宽分配、缓存决策和数据流控制决策变量集合。
...
优化问题需要确定的决策变量集合包括：
\begin{itemize}
    \item \textbf{任务分配决策} ($x_{j,n}^t$): 决定哪个任务在哪个节点上处理。
    \item \textbf{数据流控制决策} ($D_{\cdot}^t$): 决定在本地处理、卸载到RSU/UAV或在RSU间迁移的数据量。
    \item \textbf{资源管理决策} ($b_{link}^t, \mu_r^t, \nu_{r,r'}^t$): 决定通信带宽的分配以及计算单元和迁移链路的激活状态。
    \item \textbf{缓存管理决策} ($z_{j,r}^t$): 决定哪些任务结果应被缓存以备使用。
\end{itemize}
