...
# 快速测试（50轮）
python experiments/arrival_rate_analysis/run_td3_arrival_rate_sweep.py --episodes 50

# 完整实验（800轮）
python experiments/arrival_rate_analysis/run_td3_arrival_rate_sweep.py --episodes 800

# 自定义到达率范围
python experiments/arrival_rate_analysis/run_td3_arrival_rate_sweep.py --rates 1.0 2.0 3.0 --episodes 200
python experiments/run_td3_arrival_rate_sweep.py --rates 1.5  2.0 2.5 3.0 3.5 --episodes 500
# 或者从arrival_rate_analysis目录运行
cd experiments/arrival_rate_analysis
python run_td3_arrival_rate_sweep.py --episodes 50
...
# 添加项目根目录到路径
# 当前文件: experiments/arrival_rate_analysis/run_td3_arrival_rate_sweep.py
# 需要向上两级到达项目根目录
...
def run_training(arrival_rate: float, episodes: int, num_vehicles: int, 
    ...
    # 构建训练命令（使用项目根目录下的train_single_agent.py）
    ...
    cmd = [
        ...
        '--seed', str(seed)
        # 不使用--silent-mode，以便看到环境变量是否生效的打印信息
    ]
...
def extract_metrics(training_results: Dict[str, Any], arrival_rate: float) -> Dict[str, Any]:
    ...
    if 'final_performance' in training_results:
        final_perf = training_results['final_performance']
        metrics['final_evaluation'] = final_perf
