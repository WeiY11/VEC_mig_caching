...
【使用方法】
  python baseline_comparison/individual_runners/heuristic/run_random.py --episodes 200 --seed 42
"""
...
from baseline_comparison.improved_baseline_algorithms import RandomBaseline
from baseline_comparison.individual_runners.common import ResultsManager


def parse_args():
    parser = argparse.ArgumentParser(description='Random策略独立运行')
...
def main():
    ...
    if args.num_vehicles != 12:
        os.environ['TRAINING_SCENARIO_OVERRIDES'] = json.dumps({
    ...
    save_path = manager.save_results('Random', results, 'Heuristic', args.save_dir)
    manager.print_summary(results)
    
    print(f"\n✓ Random策略运行完成！结果: {save_path}")
    return 0
