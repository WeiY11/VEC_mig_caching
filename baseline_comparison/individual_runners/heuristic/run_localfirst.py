...
【使用方法】
  python baseline_comparison/individual_runners/heuristic/run_localfirst.py --episodes 200 --seed 42
"""
...
from baseline_comparison.improved_baseline_algorithms import LocalFirstBaseline
from baseline_comparison.individual_runners.common import ResultsManager


def parse_args():
    parser = argparse.ArgumentParser(description='LocalFirst策略独立运行')
...
def main():
    ...
    save_path = manager.save_results('LocalFirst', results, 'Heuristic', args.save_dir)
    manager.print_summary(results)
    
    print(f"\n✓ LocalFirst策略运行完成！结果: {save_path}")
    return 0
