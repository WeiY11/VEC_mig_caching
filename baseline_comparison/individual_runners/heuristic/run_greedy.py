...
【使用方法】
  python baseline_comparison/individual_runners/heuristic/run_greedy.py --episodes 200 --seed 42
"""
...
from baseline_comparison.improved_baseline_algorithms import GreedyBaseline
from baseline_comparison.individual_runners.common import ResultsManager


def parse_args():
    parser = argparse.ArgumentParser(description='Greedy策略独立运行')
...
def main():
    ...
    save_path = manager.save_results('Greedy', results, 'Heuristic', args.save_dir)
    manager.print_summary(results)
    
    print(f"\n✓ Greedy策略运行完成！结果: {save_path}")
    return 0
