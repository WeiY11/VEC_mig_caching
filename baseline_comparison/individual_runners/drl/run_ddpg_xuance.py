...
# -*- coding: utf-8 -*-
"""
DDPG算法独立运行脚本（基于xuance框架）

【使用方法】
  python baseline_comparison/individual_runners/drl/run_ddpg_xuance.py --episodes 200 --seed 42
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='DDPG算法独立运行（xuance）')
...
def run_ddpg_fallback(env, args, config):
    """使用项目自带DDPG实现"""
    print("\n使用兼容模式（项目自带DDPG）...")
    
    from single_agent.ddpg import DDPGEnvironment
    
    ddpg_env = DDPGEnvironment()
    episode_rewards, episode_delays, episode_energies, episode_completions = [], [], [], []
    
    start_time = time.time()
    
    for episode in range(1, args.episodes + 1):
        state = ddpg_env.reset_environment()
        episode_reward = 0.0
        
        for step in range(args.max_steps):
            action = ddpg_env.select_action(state)
            next_state, reward, done, info = ddpg_env.step(action, state)
            ddpg_env.remember(state, action, reward, next_state, done)
            
            if len(ddpg_env.memory) >= ddpg_env.batch_size:
                ddpg_env.update()
...
def main():
    ...
    config = create_xuance_config('DDPG', args.episodes, args.seed, args.num_vehicles, args.max_steps)
    env = VECGymEnv(config['env_config'])
    
    results = run_ddpg_fallback(env, args, config)
    env.close()
    
    manager = ResultsManager()
    save_path = manager.save_results('DDPG', results, 'DRL', args.save_dir)
    manager.print_summary(results)
    
    print(f"\n✓ DDPG训练完成！结果: {save_path}")
    return 0
