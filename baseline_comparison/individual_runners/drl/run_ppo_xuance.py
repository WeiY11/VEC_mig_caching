...
# -*- coding: utf-8 -*-
"""
PPO算法独立运行脚本（基于xuance框架）

【使用方法】
  python baseline_comparison/individual_runners/drl/run_ppo_xuance.py --episodes 200 --seed 42
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='PPO算法独立运行（xuance）')
...
def run_ppo_fallback(env, args, config):
    """使用项目自带PPO实现"""
    print("\n使用兼容模式（项目自带PPO）...")
    
    from single_agent.ppo import PPOEnvironment
    
    ppo_env = PPOEnvironment()
    episode_rewards, episode_delays, episode_energies, episode_completions = [], [], [], []
    
    start_time = time.time()
    
    for episode in range(1, args.episodes + 1):
        state = ppo_env.reset_environment()
        episode_reward = 0.0
        
        for step in range(args.max_steps):
            action = ppo_env.select_action(state)
            next_state, reward, done, info = ppo_env.step(action, state)
            ppo_env.remember(state, action, reward, next_state, done)
            
            if len(ppo_env.memory) >= ppo_env.batch_size:
                ppo_env.update()
...
def main():
    ...
    config = create_xuance_config('PPO', args.episodes, args.seed, args.num_vehicles, args.max_steps)
    env = VECGymEnv(config['env_config'])
    
    results = run_ppo_fallback(env, args, config)
    env.close()
    
    manager = ResultsManager()
    save_path = manager.save_results('PPO', results, 'DRL', args.save_dir)
    manager.print_summary(results)
    
    print(f"\n✓ PPO训练完成！结果: {save_path}")
    return 0
