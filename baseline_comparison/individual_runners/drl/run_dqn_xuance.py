...
# -*- coding: utf-8 -*-
"""
DQN算法独立运行脚本（基于xuance框架）

【使用方法】
  python baseline_comparison/individual_runners/drl/run_dqn_xuance.py --episodes 200 --seed 42
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='DQN算法独立运行（xuance）')
...
def run_dqn_fallback(env, args, config):
    """使用项目自带DQN实现"""
    print("\n使用兼容模式（项目自带DQN）...")
    ...
    for episode in range(1, args.episodes + 1):
        state = dqn_env.reset_environment()
        episode_reward = 0.0
        
        for step in range(args.max_steps):
            action = dqn_env.select_action(state)
            next_state, reward, done, info = dqn_env.step(action, state)
            dqn_env.remember(state, action, reward, next_state, done)
            
            if len(dqn_env.memory) >= dqn_env.batch_size:
                dqn_env.update()
...
def main():
    ...
    config = create_xuance_config('DQN', args.episodes, args.seed, args.num_vehicles, args.max_steps)
    env = VECGymEnv(config['env_config'])
    
    results = run_dqn_fallback(env, args, config)
    env.close()
    
    manager = ResultsManager()
    save_path = manager.save_results('DQN', results, 'DRL', args.save_dir)
    manager.print_summary(results)
    
    print(f"\n✓ DQN训练完成！结果: {save_path}")
    return 0
