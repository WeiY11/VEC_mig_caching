...
class OptimizedTD3Environment:
    ...
    def calculate_reward(self, system_metrics: Dict, prev_metrics: Optional[Dict] = None) -> float:
        ...
        try:
            ...
            # RSU卸载奖励：每1%的RSU处理比例获得0.08奖励（大幅提高）
            rsu_bonus = rsu_ratio * 8.0  # 超强奖励（50%占比能获得4.0奖励）
            
            # UAV卸载奖励：每1%的UAV处理比例获得0.01奖励（降低）
            uav_bonus = uav_ratio * 1.0  # 弱奖励（50%占比仅获得0.5奖励）
            
            # 本地处理惩罚：每1%的本地处理扣除0.04（增强）
            local_penalty = local_ratio * 4.0  # 超强惩罚（50%占比扣除2.0）
            ...
            # 🎯 奖励 = -成本 + 全部奖励
            # 目标: 让成本接近1.0，同时最大化RSU卸载比例
            # 最优奖励应接近 -1.0(成本) + 4.0(RSU 50%) + 0.5(UAV 10%) + 0.15(缓存) - 1.6(本地 40%) = +2.05
            # 新目标：RSU 60%时 = -1.0 + 4.8(RSU) + 0.2(UAV 20%) + 0.15 - 0.8(本地 20%) = +3.35
