...
class RLConfig:
    ...
    def __init__(self):
        ...
        # 🎯 核心奖励权重：延迟+能耗+完成率
        # 🔧 v29优化：增大权重放大策略差异信号
        #    问题：原权重(1.0, 1.0)导致奖励变化范围太小(约-0.3)
        #    解决：提高权重到(2.0, 2.0)，放大好坏策略差异
        self.reward_weight_delay = 2.0   # 🔧 v29: 1.0 → 2.0 (放大信号)
        self.reward_weight_energy = 2.0  # 🔧 v29: 1.0 → 2.0 (放大信号)
        
        # 🔥 关键修复：降低任务丢弃惩罚
        # 🔧 1.0 → 0.5 (降低惩罚，避免Agent为了保命而放弃优化)
        self.reward_penalty_dropped = 0.5
        self.completion_target = 0.95  # 🔧 0.75 → 0.95 (恢复高标准)
        
        # 🔥 启用完成率差距惩罚：让智能体关注完成率
        # 🔧 1.0 → 0.5 (降低惩罚，避免过度保守)
        self.reward_weight_completion_gap = 0.5
        # 🔥 启用数据丢失惩罚：防止智能体通过丢包作弊
        # 🔧 1.0 → 0.5 (降低惩罚)
        self.reward_weight_loss_ratio = 0.5
