...
class RLConfig:
    ...
    def __init__(self):
        ...
        # 🔧 修复：大幅提高卸载奖励，引导智能体学习卸载策略
        self.reward_weight_offload_bonus = 2.0  # 强卸载奖励（0.5→2.0），明确激励边缘卸载
        # 本地处理能耗惩罚：添加轻微惩罚，避免智能体过度依赖本地处理
        # 🔧 修复：添加本地处理惩罚，推动智能体探索卸载
        self.reward_weight_local_penalty = 0.5  # 本地处理惩罚（0.0→0.5），抑制过度本地化

        # 🎯 延时-能耗优化目标阈值（供算法动态调整）
        # 🔧 基准目标值会在训练中根据实际系统表现自动调整（最多放宽3倍）
        # 初始目标：保守估计，让系统在前几轮episode后自适应放宽
        self.latency_target = 0.8  # 初始时延目标（训练中会自动调整）
        self.latency_upper_tolerance = 2.0  # 容忍上限（相对宽松）
        self.energy_target = 3000.0  # 初始能耗目标（训练中会自动调整）
        self.energy_upper_tolerance = 6000.0  # 容忍上限（相对宽松）
